...
story.append(ref_tbl(
    ...
    [
        ['Page loads but cards give errors',
         'Not connected to EMCOMM-NET — your device is on a different network',
        ...
        ['FCC lookup returns no results',
         'FCC database not yet built, or callsign is not a US amateur license',
         'Run: sudo systemctl start fcc-refresh.service (needs internet)'],
        ['APRS map shows no stations',
         'Graywolf or YAAC not running, or no RF received yet',
         'Check Health Monitor for service status. Confirm antenna connected to IC-7300.'],
        ['Winlink form import fails',
         'Wrong file type — must be the XML attachment, not the message body',
